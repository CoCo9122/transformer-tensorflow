...
class SWEEncoder_ja:
    ...
    def encoder(self, text, clean=False):
    ...
    def decode(self, tokens, breakline='\n'):
        ...
        for i in tokens:
            ...
            if word[:6] == '<|byte' and word[-2:] == '|>': # バイト列でエンコードされた
                ...
                if len(byte_tokens) > 0: # バイト列をもとに戻す
                    words.append(bytearray(byte_tokens).decode('utf-8', error='replace'))
                    byte_tokens = []
                # トークンから文字列にをもとに戻す
